...
import type {
    MultiTurnEvalData, MultiTurnTarget, MultiTurnResult
} from "./types.ts"
 ...
 * 1. Fresh task: Users first message, check tools + order + LLM judge
 ...
 * - llmJudge: Does the final response make sens given the task and results?
 */
...
// Run the evaluation
evaluate({
    data: dataset as unknown as Array<{data: MultiTurnEvalData, target: MultiTurnTarget}>,
    executor,
    evaluators: {
 ...
 * - Three evals run on each test case
 * - Evaluators return 1 (pass) if no target to check against
 * - toolOrder checks sequence, toolAdvised checks forbidden tools. outputQuality uses LLM judge
